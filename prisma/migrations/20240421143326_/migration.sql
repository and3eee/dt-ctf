...
CREATE TABLE `Riddle` (
    ...
    `showRiddleResource` BOOLEAN NOT NULL DEFAULT false,
...
-- CreateTable
CREATE TABLE `RiddleResource` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `name` VARCHAR(191) NOT NULL,
    `description` VARCHAR(191) NULL,
    `link` VARCHAR(191) NULL,
    `AuthInfo` VARCHAR(191) NULL,
    `owner` VARCHAR(191) NULL,

    UNIQUE INDEX `RiddleResource_id_key`(`id`),
    UNIQUE INDEX `RiddleResource_name_key`(`name`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- CreateTable
CREATE TABLE `_RiddleToRiddleResource` (
    `A` INTEGER NOT NULL,
    `B` INTEGER NOT NULL,

    UNIQUE INDEX `_RiddleToRiddleResource_AB_unique`(`A`, `B`),
    INDEX `_RiddleToRiddleResource_B_index`(`B`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `_RiddleToRiddleResource` ADD CONSTRAINT `_RiddleToRiddleResource_A_fkey` FOREIGN KEY (`A`) REFERENCES `Riddle`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `_RiddleToRiddleResource` ADD CONSTRAINT `_RiddleToRiddleResource_B_fkey` FOREIGN KEY (`B`) REFERENCES `RiddleResource`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
